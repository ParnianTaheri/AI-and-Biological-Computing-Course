...
function [Feature_best_ind, j_best, Feature_best2,j_best2, Normalized_Train_Features, listOfChannels] = feature_extraction(ch, x_train, y_train, Fs, train_size, num_feature)
...
% skewness_signal = zeros(train_size,ch);
% for j=1:ch 
%     for i=1:train_size
%       skewness_signal(i,j) = skewness(squeeze(x_train(j,:,i)));
%     end
%      skewness_signal_J(ch) = fisher(y_train, ch, skewness_signal); 
% end
% 
% disp(['Fisher score for best skewness = ', num2str(max(skewness_signal_J)), ' for channel = ', num2str(find(skewness_signal_J == max(skewness_signal_J)))]);
% J_mat = [J_mat;skewness_signal_J];
% feature_num = feature_num + 1;
%Train_Features(:,:,feature_num) = skewness_signal;

% % Entropy
% disp("Calculating Entropy")
% Entropy = zeros(train_size,ch);
% for j=1:ch
%     for i=1:train_size
%       Entropy(i,j) = entropy(squeeze(x_train(j,:,i)));
%     end
%     Entropy_J(ch) = fisher(y_train, ch, Entropy);
% end
% disp(['Fisher score for best entropy = ', num2str(max(Entropy_J)), ' for channel = ', num2str(find(Entropy_J == max(Entropy_J)))]);
% J_mat = [J_mat;Entropy_J];
% feature_num = feature_num + 1;
%Train_Features(:,:,feature_num) = Entropy;
...
j_best2 = J(I2);
Feature_best2 = Normalized_Train_Features(:,I2);

%% fisher multi variable
...
j_best = 0;
for i = 1:1500
    y = randsample(n,num_feature);
size(I)
    in = I(y,1);  

   
    u1 = mean(Normalized_Train_Features(class1_indices,in)) ;
    
    S1 = (Normalized_Train_Features(class1_indices,in) - u1)'*(Normalized_Train_Features(class1_indices,in) - u1);
    S1 = S1/length(class1_indices);
 
    u2 = mean(Normalized_Train_Features(class0_indices,in)) ;
    S2 = (Normalized_Train_Features(class0_indices,in)-u2)'*(Normalized_Train_Features(class0_indices,in) - u2);
    S2 = S2/length(class0_indices);

    Sw = S1 + S2 ;

    u0 = mean(Normalized_Train_Features(:,in)) ; 
    Sb1 = (u1 - u0)'*(u1 - u0) ;
    Sb2 = (u2 - u0)'*(u2 - u0) ;
...
disp("End of Featrue Extraction")
